#!/usr/bin/env node
import { getReceiptsDir, getRunId } from "@qbos/run";
...
const RUN_ID =
  process.env.ROBBY_RUN_ID || process.env.QBOS_RUN_ID || getRunId();
process.env.ROBBY_RUN_ID = RUN_ID;
process.env.QBOS_RUN_ID = process.env.QBOS_RUN_ID || RUN_ID;

// Receipts directory: per-run folder to avoid cross-run collisions
const resolvedReceiptsDir =
  process.env.ROBBY_RECEIPTS_DIR || getReceiptsDir("robby-pa");
const RECEIPTS_DIR = path.resolve(process.cwd(), resolvedReceiptsDir);
process.env.ROBBY_RECEIPTS_DIR = RECEIPTS_DIR;
process.env.QBOS_RECEIPTS_ROOT =
  process.env.QBOS_RECEIPTS_ROOT || path.dirname(path.dirname(RECEIPTS_DIR));
...
async function writeReceipt(obj) {
  ...
  const file = path.join(
    RECEIPTS_DIR,
    `${String(seq).padStart(4, "0")}_${obj.type}_${Date.now()}.json`
  );
...
async function runCommand(stepName, cmd, args, opts = {}) {
  ...
  const stdoutPath = path.join(
    RECEIPTS_DIR,
    `${String(seq).padStart(4, "0")}_${stepName}_${stamp}.stdout.log`
  );
  const stderrPath = path.join(
    RECEIPTS_DIR,
    `${String(seq).padStart(4, "0")}_${stepName}_${stamp}.stderr.log`
  );
  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", async (d) => {
      await outStream.write(d);
    });
    child.stderr.on("data", async (d) => {
      await errStream.write(d);
    });
...
async function main() {
  ...
  const steps = [
    {
      name: "unit-tests",
      cmd: "pnpm",
      args: ["-C", "packages/robby-pa", "run", "test:unit"],
    },
    {
      name: "migrate",
      cmd: "node",
      args: ["packages/robby-pa/bin/migrate.cjs"],
    },
    {
      name: "integration-tests",
      cmd: "pnpm",
      args: ["-C", "packages/robby-pa", "run", "test:integration"],
    },
    {
      name: "teardown-postgres",
      cmd: "docker",
      args: ["rm", "-f", "robby_pa_dev_postgres"],
    },
  ];
  ...
  for (const s of steps) {
    ...
    if (
      s.name.startsWith("start-postgres") ||
      s.name.startsWith("teardown-postgres")
    ) {
      try {
  ...
  try {
    const chainOk = await verifyReceipts(
      RECEIPTS_DIR,
      process.env.ROBBY_RECEIPT_MAC_SECRET
    );
    if (!chainOk) {
      const r = {
        seq,
      ...
      await writeReceipt(r);
      console.error(
        "Receipt chain verification failed — aborting (fail-closed)"
      );
      process.exit(5);
    }
  } catch (e) {
...
async function verifyReceipts(dir, secret) {
  ...
  for (let i = 0; i < receipts.length; i++) {
    ...
    const computedHash = crypto
      .createHash("sha256")
      .update(JSON.stringify(withoutMac, null, 2))
      .digest("hex");
    ...
    if (r.mac && r.mac.value) {
      if (r.mac.value !== expectedMac) {
        console.error(
          "verifyReceipts: mac mismatch at seq",
          r.seq,
          "expected",
          expectedMac,
          "got",
    ...
    if (i === 0) {
      ...
      if (r.prev_hash !== prev.hash) {
        console.error(
          "verifyReceipts: prev_hash mismatch at seq",
          r.seq,
          "expected",
          prev.hash,
          "got",
