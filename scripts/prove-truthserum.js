...
async function run() {
  ...
  await TS.ReceiptWriter.writeReceipt({ type: 'identity.authenticated', details: { user: 'maintainer' } });
  await TS.ReceiptWriter.writeReceipt({ type: 'billing.cap_not_exceeded', details: { cap: true } });
  await TS.ReceiptWriter.writeReceipt({ type: 'safety.passed', details: { ok: true } });

  // also write session.ready proofs
  await TS.ReceiptWriter.writeReceipt({ type: 'session.created', details: { sessionId: 's-demo' } });
  await TS.ReceiptWriter.writeReceipt({ type: 'billing.active', details: { plan: 'pro' } });
