...
async function POST(request) {
    try {
        ...
        if (!template_id) {
            return server_1.NextResponse.json({ error: 'template_id required' }, { status: 400 });
        }
        ...
        if (!supabaseUrl || !supabaseKey) {
            ...
            return server_1.NextResponse.json({
                ...
                messages: [
                    {
                        role: 'system',
                        content: '⚠️ Running in mock mode (Supabase not configured). Receipts will not persist.',
                        timestamp: new Date().toISOString(),
                    },
                    {
                        role: 'assistant',
                        content: "Hello! I'm Rob the QuietBuilder. Before we begin, I need your consent to proceed. Please type 'I consent' to continue.",
                        timestamp: new Date().toISOString(),
                    },
                ],
