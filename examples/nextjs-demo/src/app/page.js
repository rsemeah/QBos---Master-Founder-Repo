...
function Home() {
    ...
    const [aiResponse, setAiResponse] = (0, react_1.useState)(null);
    const [aiLoading, setAiLoading] = (0, react_1.useState)(false);
    const [tier, setTier] = (0, react_1.useState)('A');
    const [validationResult, setValidationResult] = (0, react_1.useState)(null);
    const [validationLoading, setValidationLoading] = (0, react_1.useState)(false);
    ...
    return (<main className="min-h-screen p-8 bg-gradient-to-br from-gray-900 via-black to-gray-900">
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={aiInput} onChange={(e) => setAiInput(e.target.value)} placeholder="Explain quantum computing..." className="w-full p-3 bg-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:border-blue-500 resize-none" rows={4}/>
              </div>

              <button onClick={handleAiGenerate} disabled={aiLoading || !aiInput.trim()} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 disabled:cursor-not-allowed py-3 px-4 rounded-lg font-medium transition-colors">
                {aiLoading ? 'Generating...' : 'Generate Response'}
              </button>

              {aiResponse && (<div className="mt-4 p-4 bg-gray-900 border border-gray-700 rounded-lg">
                  {aiResponse.error ? (<p className="text-red-400">{aiResponse.error}</p>) : (<>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {['A', 'B', 'C'].map((t) => (<button key={t} onClick={() => setTier(t)} className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${tier === t
              ...
              <button onClick={handleValidation} disabled={validationLoading} className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 disabled:cursor-not-allowed py-3 px-4 rounded-lg font-medium transition-colors">
                {validationLoading ? 'Validating...' : 'Validate Asset'}
              </button>

              {validationResult && (<div className="mt-4 p-4 bg-gray-900 border border-gray-700 rounded-lg">
                  {validationResult.error ? (<p className="text-red-400">{validationResult.error}</p>) : (<>
                      <div className="flex items-center justify-between mb-3">
                        <span className={`text-lg font-semibold ${validationResult.passed
                    ? 'text-green-400'
                    : 'text-red-400'}`}>
                    ...
                    validationResult.issues.length > 0 && (<div className="pt-3 border-t border-gray-700">
                            <p className="text-sm font-medium mb-2 text-gray-400">
                              Issues:
                            </p>
                            <ul className="text-sm space-y-1">
                              {validationResult.issues.map((issue, i) => (<li key={i} className="text-yellow-400">
                                  • {issue}
                                </li>))}
                            </ul>
                          </div>)}
