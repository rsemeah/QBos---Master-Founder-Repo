...
exports.default = EnginesIndexPage;
...
function EnginesIndexPage() {
    return (<div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-12">
        {/* Header */}
        <div className="mb-8">
          <link_1.default href="/" className="text-blue-600 hover:text-blue-700 text-sm font-medium mb-4 inline-block">
            ← Back to Home
          </link_1.default>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ENGINES.map((engine) => (<link_1.default key={engine.key} href={`/engines/${engine.key}`} className="block bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md hover:border-blue-300 transition-all">
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-lg font-bold text-gray-900">{engine.name}</h3>
                <span className={`px-2 py-1 rounded text-xs font-semibold ${engine.status === 'operational'
