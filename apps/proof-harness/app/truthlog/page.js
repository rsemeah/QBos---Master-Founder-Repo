...
exports.default = TruthLogPage;
const react_1 = require("react");
const link_1 = __importDefault(require("next/link"));
function TruthLogPage() {
    ...
    const [selectedEngines, setSelectedEngines] = (0, react_1.useState)(['rob']);
    ...
    const toggleEngine = (engine) => {
        setSelectedEngines(prev => prev.includes(engine)
            ? prev.filter(e => e !== engine)
            : [...prev, engine]);
    };
    const getReceiptColor = (type) => {
        if (type.startsWith('rob.'))
            return 'bg-purple-100 text-purple-800 border-purple-300';
        if (type.startsWith('silent.'))
            return 'bg-blue-100 text-blue-800 border-blue-300';
        if (type.startsWith('identity.'))
            return 'bg-green-100 text-green-800 border-green-300';
        if (type.startsWith('charter.'))
    ...
    return (<div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {ENGINE_PREFIXES.map((engine) => (<button key={engine} onClick={() => toggleEngine(engine)} className={`px-4 py-2 rounded-lg border transition-colors ${selectedEngines.includes(engine)
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {!loading && !error && receipts.length === 0 && (<div className="text-center py-8 text-gray-500">
              No receipts found. Start using Rob to see operations appear here.
            </div>)}

          {!loading && !error && receipts.length > 0 && (<div className="space-y-4">
              {receipts.map((receipt, index) => (<div key={receipt.id} className="border-l-4 border-blue-500 pl-4 py-2 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`inline-block px-3 py-1 text-xs font-semibold rounded-full border ${getReceiptColor(receipt.type)}`}>
                          {receipt.type}
                        </span>
                      ...
                      <div className="text-sm text-gray-700 mb-2">
                        {Object.keys(receipt.details).length > 0 && (<pre className="bg-gray-50 p-2 rounded text-xs overflow-x-auto">
                      ...
                      <div className="text-xs text-gray-500 space-y-1">
                        {receipt.caused_by_message_id && (<div>Caused by message: {receipt.caused_by_message_id.slice(0, 8)}...</div>)}
                        {receipt.caused_by_rule && (<div>Rule: {receipt.caused_by_rule}</div>)}
                        {receipt.caused_by_constraint && (<div>Constraint: {receipt.caused_by_constraint}</div>)}
                      </div>
                    </div>
