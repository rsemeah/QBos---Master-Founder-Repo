...
const ai_service_1 = require("../../../lib/ai-service");
...
async function POST(request) {
    try {
        ...
        if (!session_id || !message) {
            return server_1.NextResponse.json({ error: 'session_id and message required' }, { status: 400 });
        }
        ...
        if (!consentGranted) {
            if (messageLC.includes('consent') || messageLC.includes('agree')) {
                ...
                const response = "✅ Consent granted. I can now help you build. What would you like to create?";
                await supabase.from('rob_messages').insert({
                    session_id,
                    role: 'assistant',
                    content: response,
                ...
                const response = "⚠️ I need your consent to proceed. Please type 'I consent' to continue.";
                await supabase.from('rob_messages').insert({
                    session_id,
                    role: 'assistant',
                    content: response,
        ...
        const aiResult = await (0, ai_service_1.generateCodeWithAI)({
...
function handleMockMode(session_id, message) {
    const messageLC = message.toLowerCase();
    if (messageLC.includes('consent') || messageLC.includes('agree')) {
        return server_1.NextResponse.json({
            ok: true,
