...
export function validateAsset(
  ...
  const tierSpec = QUALITY_TIER_SPECS[tier];
  const errors: string[] = [];
  const warnings: string[] = [];
  const rejections: Partial<RejectionCriteria> = {};
  ...
  if (!tierSpec.requiredColorSpace.includes(spec.colorSpace)) {
    errors.push(
      `Color space ${spec.colorSpace} not in required: ${tierSpec.requiredColorSpace.join(', ')}`
    );
    score -= 15;
  }
...
export function validateResolution(
  ...
  if (resolution.width < minRes.width || resolution.height < minRes.height) {
    errors.push(
      `Resolution ${resolution.width}×${resolution.height} below minimum ${minRes.width}×${minRes.height} for tier ${tierSpec.tier}`
    );
  }
 ...
 */
export function validateCameraSpec(
  camera: { model: string; lens: string; aperture: number },
  tierSpec: QualityTierSpec
): { passed: boolean; warnings: string[] } {
  const warnings: string[] = [];

  // Check camera model
  if (!tierSpec.cameraModels.includes(camera.model as any)) {
    warnings.push(
      `Camera model ${camera.model} not in recommended list for tier ${tierSpec.tier}`
    );
  }

  // Check lens type
  if (!tierSpec.lensTypes.includes(camera.lens as any)) {
  ...
  if (camera.aperture < minAperture || camera.aperture > maxAperture) {
    warnings.push(
      `Aperture f/${camera.aperture} outside recommended range f/${minAperture} - f/${maxAperture}`
    );
  }
...
export function validateLighting(
  lighting: { setup: string; keyLight: any; colorTemperature: number },
  ...
  if (lighting.colorTemperature < 2500 || lighting.colorTemperature > 7000) {
    errors.push(
      `Color temperature ${lighting.colorTemperature}K outside acceptable range (2500-7000K)`
    );
  }
...
export function validateComposition(
  composition: { rules: string[]; silhouetteClarity: number; visualClutter: number },
  tierSpec: QualityTierSpec
): { passed: boolean; warnings: string[] } {
  const warnings: string[] = [];
...
export function validateVideoSpec(
  video: { frameRate: number; codec?: string },
  tierSpec: QualityTierSpec
): { passed: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
export function validateLogoRequirements(logo: LogoRequirements): {
  passed: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function checkAIArtifacts(spec: AssetSpec): RejectionCriteria {
  const rejections: RejectionCriteria = {
 ...
 */
export function calculateQualityScore(spec: AssetSpec, tier: QualityTier): number {
 ...
 */
export function generatePromptHeader(tier: QualityTier = 'A'): string {
  const tierSpec = QUALITY_TIER_SPECS[tier];

  const cameraSpec = `${tierSpec.cameraModels[0]}, ${tierSpec.lensTypes[0]}, f/${tierSpec.apertureRange[0]}`;
  const lightingSpec =
    tierSpec.lightingRequirement === 'cinematic'
 ...
 */
export function validateAssetPipeline(assets: {
  spec: AssetSpec;
  type: AssetType;
  tier: QualityTier;
}[]): {
