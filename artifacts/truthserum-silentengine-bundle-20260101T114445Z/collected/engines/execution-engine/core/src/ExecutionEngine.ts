...
import type { BuildSession, BuildStep, ExecutionEngineResult, ReceiptsBundle } from './types';
...
export class ExecutionEngine {
  ...
  private executingSteps: Set<string> = new Set(); // Prevent concurrent execution
  ...
  async createBuildSession(
    ...
    for (const def of stepDefinitions) {
      if (def.goalTags.includes('*')) continue; // Already added universal
      session = addStepToSession(session, {
        id: def.id,
        title: def.title,
   ...
   */
  async getNextStep(sessionId: string): Promise<ExecutionEngineResult<BuildStep>> {
   ...
   */
  async executeStep(sessionId: string, stepId: string): Promise<ExecutionEngineResult<BuildStep>> {
    const session = this.stateStore.getSession(sessionId);
    if (!session) {
      return {
        ok: false,
    ...
    try {
      ...
      const updatedSession = updateStepResult(session, stepId, result);
      this.stateStore.saveSession(updatedSession);

      // Check if all steps done
      if (updatedSession.currentStepIndex >= updatedSession.steps.length) {
        const completedSession = markSessionComplete(updatedSession);
      ...
      const updatedSession = updateStepResult(session, stepId, failedResult);
      this.stateStore.saveSession(updatedSession);
   ...
   */
  async getReceipts(sessionId: string): Promise<ExecutionEngineResult<ReceiptsBundle>> {
    const session = this.stateStore.getSession(sessionId);
    if (!session) {
      return {
        ok: false,
    ...
    const receipts = generateReceipts(session);
    return {
      ok: true,
      data: receipts,
    };
