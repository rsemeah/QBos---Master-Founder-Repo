...
export interface BuildStep {
  id: string; // STABLE SEMANTIC ID (e.g. "identity.check.users")
  title: string;
  explanation: string; // child-readable
...
export interface StepResult {
  ok: boolean;
  summary: string; // friendly explanation
  evidence?: Evidence[];
  artifacts?: Artifact[];
  warnings?: string[];
}
...
export interface Evidence {
  ...
  description: string;
  reference: string; // path, key, or identifier
  hash?: string;
}
