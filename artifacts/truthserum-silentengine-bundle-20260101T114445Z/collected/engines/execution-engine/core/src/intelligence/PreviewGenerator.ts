...
export class PreviewGenerator {
  ...
  async generate(
    ...
    const domain = (ideaReceipt.details as any).detectedDomain;
    const coreMechanics = (conceptReceipt.details as any).coreMechanics || [];

    // Select template based on domain/mechanics
    let template: { name: string; code: string };
