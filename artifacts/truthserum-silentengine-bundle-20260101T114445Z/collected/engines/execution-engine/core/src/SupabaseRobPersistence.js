...
exports.SupabaseRobPersistence = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
class SupabaseRobPersistence {
    supabase;
    ...
    async addReceipt(receipt) {
        ...
        return {
            ...
            actor: data.details.actor,
            action_type: data.type,
            outcome: data.details.outcome,
            evidence_refs: data.details.evidence_refs,
            truth_state: data.details.truth_state,
            metadata: data.details.metadata,
            parent_receipt_id: data.details.parent_receipt_id,
        };
    }
    async getReceipts(sessionId) {
        ...
        return data.map((row) => ({
            ...
            actor: row.details.actor,
            action_type: row.type,
            outcome: row.details.outcome,
            evidence_refs: row.details.evidence_refs,
            truth_state: row.details.truth_state,
            metadata: row.details.metadata,
            parent_receipt_id: row.details.parent_receipt_id,
        }));
    }
    // ==========================================================================
    // STATE TRANSITIONS
    // ==========================================================================
    async addStateTransition(sessionId, transition) {
        ...
        if (error)
            throw new Error(`Failed to add state transition: ${error.message}`);
    }
    async getStateTransitions(sessionId) {
        const { data, error } = await this.supabase
            ...
            .order('created_at', { ascending: true });
        if (error)
            throw new Error(`Failed to get state transitions: ${error.message}`);
        return data.map((row) => ({
    ...
    // CONFIG HISTORY
    // ==========================================================================
    async addConfigChange(sessionId, change) {
        ...
        if (error)
            throw new Error(`Failed to add config change: ${error.message}`);
    }
    ...
    // UNDO STACK
    // ==========================================================================
    async pushUndoSnapshot(sessionId, snapshot) {
        ...
        if (error)
            throw new Error(`Failed to push undo snapshot: ${error.message}`);
    }
    async popUndoSnapshot(sessionId) {
        ...
        const { data: snapshots, error: fetchError } = await this.supabase
            ...
            .limit(1);
        if (fetchError)
            throw new Error(`Failed to fetch undo snapshot: ${fetchError.message}`);
        ...
        const { error: deleteError } = await this.supabase
            ...
            .eq('id', snapshot.id);
        if (deleteError)
            throw new Error(`Failed to delete undo snapshot: ${deleteError.message}`);
        return {
            action_type: snapshot.action_type,
            snapshot: snapshot.snapshot,
        };
...
exports.SupabaseRobPersistence = SupabaseRobPersistence;
//# sourceMappingURL=SupabaseRobPersistence.js.map