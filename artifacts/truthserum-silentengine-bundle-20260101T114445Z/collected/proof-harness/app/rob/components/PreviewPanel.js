...
function PreviewPanel({ receipts }) {
    const latestPreview = receipts.find((receipt) => receipt.type?.includes('preview'));
    return (<div className="flex-1 border-b border-gray-200 bg-white p-6">
      <h2 className="text-lg font-semibold text-gray-900 mb-2">Live Preview</h2>
      {latestPreview ? (<div className="text-sm text-gray-600">
        ...
        </div>) : (<p className="text-sm text-gray-500">No preview generated yet.</p>)}
    </div>);
}
