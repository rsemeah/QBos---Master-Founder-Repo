...
function EnginePage({ params }) {
    const engineKey = params.engineKey;
    const [receipts, setReceipts] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    if (!VALID_ENGINE_KEYS.includes(engineKey)) {
    ...
    async function fetchEngineReceipts() {
        try {
            ...
            const filtered = (data.receipts || []).filter((r) => engineReceiptTypes.some(type => r.type.startsWith(type)));
    ...
    function getEngineReceiptTypes(key) {
        const typeMap = {
    ...
    return (<main style={{ fontFamily: 'system-ui', padding: '3rem', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ marginBottom: '2rem', display: 'flex', gap: '1rem' }}>
        <link_1.default href="/engines" style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '2rem' }}>
          <span style={{
            ...
            padding: '0.5rem 1rem',
            borderRadius: '6px',
            backgroundColor: engine.status === 'operational'
                ? '#22c55e'
        ...
        <section style={{ marginBottom: '2rem' }}>
          ...
          {loading ? (<p style={{ color: '#666' }}>Loading receipts...</p>) : receipts.length > 0 ? (<div>
              <p style={{ color: '#333', marginBottom: '1rem' }}>
                Found {receipts.length} receipt(s) for this engine:
              </p>
              <ul style={{ lineHeight: '1.8', color: '#333' }}>
                {receipts.slice(0, 5).map((receipt, idx) => (<li key={idx}>
              ...
              {receipts.length > 5 && (<p style={{ color: '#666', marginTop: '0.5rem', fontSize: '0.9rem' }}>
                  ... and {receipts.length - 5} more
                </p>)}
            </div>) : (<p style={{ color: '#ef4444' }}>
              No receipts found for this engine. Status: <strong>Unknown</strong>
            </p>)}
          
          {engine.status === 'operational' && (<div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#f0fdf4', borderRadius: '6px' }}>
              <h3 style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>Core Capabilities</h3>
              <ul style={{ lineHeight: '1.8', color: '#333' }}>
                {engine.capabilities.map((cap, idx) => (<li key={idx}>{cap}</li>))}
              </ul>
            </div>)}
        </section>
